...
| ---- | ----- | ---- | ---- | -------- | ----- | ------ | ----- |
| 2025-12-12 | 4.1 | 3 | Bug | High | TBD | Addressed | `extracted_text` column in `public.study_materials` is `NULL` for uploaded PDF/text files. This is due to the `pdf-parser` being a simulation (`src/app/api/pdf-parser/route.ts`) and the extracted text not being saved back to the database. This prevents actual summary/quiz generation with real documents. 
