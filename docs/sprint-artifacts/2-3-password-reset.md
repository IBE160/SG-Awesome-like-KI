...
- [ ] **Task 1: Implement "Forgot Password" UI (AC: All)**
  - [ ] Add "Forgot Password" link on the login page.
  - [ ] Create a UI for entering email address for password reset.
  - [ ] Display messages to the user (e.g., "Check your email", "Link expired").
  - [ ] Create a UI for setting a new password.
  - [ ] Implement client-side validation for new password strength.
- [ ] **Task 2: Implement Password Reset API Endpoints (AC: All)**
  - [ ] Create a Next.js API route to initiate password reset (e.g., `src/app/api/auth/reset-password/request/route.ts`).
  - [ ] Call Supabase Auth function to send password reset email.
  - [ ] Create a Next.js API route to handle password update (e.g., `src/app/api/auth/reset-password/confirm/route.ts`).
  - [ ] Call Supabase Auth function to update user's password.
  - [ ] Implement server-side validation for new password strength.
- [ ] **Task 3: Testing (AC: All)**
  ...
  - [ ] Write end-to-end tests for the complete password reset flow (dependent on registration and login).
  - [ ] Manual testing of UI/UX, email delivery, and link expiration.
