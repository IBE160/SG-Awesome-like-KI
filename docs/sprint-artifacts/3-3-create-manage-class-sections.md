...
- [ ] Implement UI for creating, renaming, and deleting class sections (AC: 1)
  - [ ] Implement form for creating new sections (input field for name, create button)
  - [ ] Implement UI for listing existing sections within a class with options to rename and delete
  - [ ] Implement confirmation dialog for section deletion (AC: 2)
  - [ ] Implement client-side validation for section name (25 alphanumeric characters, AC: 3)
- [ ] Implement API endpoints for class section management (AC: 1, 3, 4)
  - [ ] `POST /api/classes/{id}/sections` for creating sections
  - [ ] `PUT /api/sections/{id}` for renaming sections
  - [ ] `DELETE /api/sections/{id}` for deleting sections
  - [ ] Implement server-side validation for section name (25 alphanumeric characters, uniqueness within class)
  - [ ] Implement logic for cascading deletion of associated content (AC: 2)
- [ ] Implement `GET /api/classes/{id}/sections` endpoint for retrieving a class's sections (AC: 1)
...
*   `src/app/api/classes/[id]/sections/route.ts` (NEW) - for the class sections API endpoint (POST)
...
*   `src/lib/supabase/client.ts` (MODIFIED) - for Supabase client integration (class_sections table operations)
